...
https://growthbeat.com/ より、会員登録を行うことができます。右上の【会員登録(無料)】をクリックしていただきますと、以下の画像に示されている画面に遷移してアカウント作成が行えます。メールアドレス・パスワード・お客様の属性を選択された後に、利用規約をご覧ください。利用規約とお知らせメールの受信に同意していただける場合にはチェックをしていただきまして、【無料登録】をクリックしていただけますと新規登録は完了です。
...
上記の操作を完了すると、管理画面が表示されます。この時アプリは Default App が選択された状態になっています。
...
GrowthMessageに登録されているイベントを一覧で表示する画面です。一覧項目は以下の3つになっており、詳細ボタンをクリックするとそれぞれのイベントを起こしたユーザーのログを確認することができます。
...
# 配信作成
## 配信名 / 説明
配信作成をするために、まずは対象となるアプリをメインメニューの【アプリ】から選択 してください。その後、メインメニューから【配信作成】を選択します。 配信名には、送信したいメッセージのタイトルを入力してください(例: 課金訴求ポップアップ)。配信名の最大文字数は 32 文字です。タスク説明には、送信したいメッセージについての補足説明を入力してください。

<img src="/img/message/message-create.png" alt="message-create" title="message-create" width="100%"/>

## 配信メッセージ

配信するメッセージのタイプを指定することができます。配信のタイプは 1,テキスト型 2,カード型 3,バナー型 4,ウォークスルー型 の4種が用意されています。パターンは任意で4種まで追加していくことが可能です。追加するには【+パターンを追加】ボタンをクリックしてください。

### テキスト型

テキストを使ったダイアログの表示方法です。入力項目はタイトル・本文・ボタンのテキスト及びクリック時のアクションとなります。アクションはデフォルトでは登録されていないため、登録する必要があります。
...
<img src="/img/message/message-add-action01.png" alt="message-add-action01" title="message-add-action01" width="100%"/>
...
**AndroidとiOSの表示の違い**
Androidのみ、バナーを閉じるボタンを押さないと、アプリ終了後も画面上に表示された状態になります。

* アイコン画像推奨サイズ

```
推奨サイズ:横 120(px)、縦 120(px)
```
...
<img src="/img/message/message-preview-device.png" alt="message-preview-device" title="message-preview-device" width="100%"/>
...
* xx日間連続でログインしているユーザー
  * メッセージが配信される当日が、連続ログインxx日目にあたるユーザーを指します。
* 休眠ユーザー
...
* xx日前にインストールしたユーザー
  * メッセージが配信されるxx日前に、アプリをインストールしたユーザーを指します。
...
有効期間の設定で、メッセージが表示される期間を指定することができます。期間中であっても配信を一時停止することは可能です。停止の仕方については、【[配信設定](#配信設定)】をご参照ください。
...
同時に複数のメッセージを配信状態にすると、**配信中の設定のうち、どれか1つのメッセージがシステム内でランダムに選択され、対象のユーザーにメッセージを配信する**仕様になっております。
...
<img src="/img/message/message-edit-action.png" alt="message-edit-action" title="message-edit-action" width="100%"/>

## アクションの追加
新しいアクションを追加することができます。ここで作成したアクションは、ユーザーがメッセージ上のボタンをクリックした後に起こるアクションとして利用します。

<img src="/img/message/message-add-action02.png" alt="message-add-action" title="message-add-action" width="100%"/>
...
デバイス名は、テストデバイスを判別するための説明を自由に入力することができます。ユーザー名には実装時に設定したユニークなユーザー値を入力する必要があります。実装に関しては [iOSはこちら](/sdk/ios/reference/#特定のタグを送信) 、 [Androidはこちら](/sdk/android/reference/#特定のタグを送信) を参照してください。

正常に登録が完了しておりますと、下記ページにて設定した値を確認ができます。

基本設定 > タグ設定 > ID: `Tag:${ApplicationId}:Default:UserID` 表示名:ユーザーID > 詳細
